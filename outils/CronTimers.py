...
class CronTimers(Screen):
    ...
    def checkNetworkStateFinished(self, result, retval, extra_args = None):
        if 'bad address' in result:
            self.session.openWithCallback(self.InstallPackageFailed, MessageBox, _('Your device is not connected to the internet, please check your network settings and try again.'), type=MessageBox.TYPE_INFO, timeout=10, close_on_any_key=True)
